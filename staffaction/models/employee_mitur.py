...
class employee_mitur(models.Model):
    _name = 'employee.mitur'
    _description = ''

    name = fields.Char("Cedula", required=True)
    last = fields.Char("Apellidos")
    ...
    def using_api_external(self, cedula):
        if cedula:
            try:
                ...
                api_url = (
                    self.env['ir.config_parameter'].sudo().get_param('api.url') + cedula)               
    ...
    def create(self, values):
        ...
        return super(employee_mitur, self).create(values)
