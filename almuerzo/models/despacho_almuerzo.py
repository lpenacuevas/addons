    ...
    def update_lunch_parameters(self):
        ...
        for rec in self:
            ...
            employee_obj = self.env['hr.employee'].search(
                [('identification_id', '=', formatted_cedula), ('identification_id', '!=', '')], limit=1)
            contact_records = self.env['almuerzo.almuerzo'].search([('cedula', '=', formatted_cedula)])
            contact_obj = contact_records.sorted(key=lambda r: r.create_date, reverse=True)[
                0] if contact_records else None

            if formatted_cedula:
                # Check if the employee is allowed to eat
                employee_is_allowed_to_eat = True if employee_obj and employee_obj.enter_date and (
                        (not employee_obj.end_date) or (
                        employee_obj.end_date and datetime.now().date() <= employee_obj.end_date)) else False
                ...
                if not (employee_is_allowed_to_eat or contact_is_allowed_to_eat):
                    raise ValidationError(
                        "Cédula no encontrada. Por favor, ingrese una cédula válida o póngase en contacto con el administrador del sistema.")
